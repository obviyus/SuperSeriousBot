...
def audio(update: 'telegram.Update', context: 'telegram.ext.CallbackContext') -> None:
    ...
    if update.message.caption:
        audio_choice = list(message.parse_caption_entities([MessageEntity.BOT_COMMAND]).values())[0]
    elif update.message.text:
        audio_choice = list(message.parse_entities([MessageEntity.BOT_COMMAND]).values())[0]
    else:
        return
    ...
    if (
        ...
        if message.reply_to_message:
            message.reply_to_message.reply_voice(voice=data[file_id_names[audio_choice]])
        else:
