...
def tldr(update: "telegram.Update", _context: "telegram.ext.CallbackContext") -> None:
    ...
    try:
        ...
        if (
            len(message.reply_to_message.parse_entities([MessageEntity.URL]).values())
            == 1
        ):
            content_url = message.reply_to_message.parse_entities(
                [MessageEntity.URL]
            ).values()
    ...
    # If parent message is empty
        ...
        text = (
            "*Usage:* `/tldr` in reply to a message or link.\n\n"
