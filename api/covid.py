...
def total(buffer: io.BytesIO, days: int) -> None:
    ...
    index = pd.date_range(
        start=df["Date_YMD"][0], end=df["Date_YMD"][len(df) - 1], freq="D"
    )
    ...
    ax = df.plot(
        y=["Daily Confirmed", "Daily Recovered", "Daily Deceased"],
        kind="line",
        linewidth=2.0,
    )
...
def top_states(buffer: io.BytesIO, days: int) -> None:
    plt.rcParams["axes.prop_cycle"] = plt.cycler(
        color=["#6886c5", "#f1935c", "#a1cae2", "#cd5d7d", "#838383"]
    )
    ...
    confirmed: pd.DataFrame = confirmed[
        confirmed.select_dtypes(include=[np.number]).ge(0).all(1)
    ]
