...
def construct_message(
    magnet_list: List[str], desc_list: List[str], min_offset: int
) -> Tuple[str, InlineKeyboardMarkup]:
    max_offset: int = min(min_offset + items_per_page, len(magnet_list))
...
def search(update: "telegram.Update", context: "telegram.ext.CallbackContext") -> None:
    ...
    if context.chat_data is not None:
        ...
        sent_message: "telegram.Message" = message.reply_text(
            text="Fetching results...", quote=True, parse_mode="HTML"
        )
        context.chat_data["search_msg_id"] = sent_message.message_id

        search_job: qbittorrentapi.search = qbt_client.search_start(
            pattern=query, category="all", plugins="all"
        )

        while (
            search_job.status()[0].total <= total_items
            and search_job.status()[0].status != "Stopped"
        ):
            time.sleep(0.1)
        ...
        search_results: list = list(
            search_job.results(limit=total_items, offset=1).results
        )
        search_job.delete()
        ...
        context.chat_data["magnet_list"] = [
            f"{result.fileName}:\n\n{result.fileUrl}" for result in search_results
        ]
        ...
        if not context.chat_data["desc_list"]:
            ...
            sent_message.edit_text(
                text=f"Page 1:\n\n{text}", reply_markup=kb_menu, parse_mode="HTML"
            )
    ...
    if index == -1:  # next page
        if offset + items_per_page >= len(magnet_list):
            ...
            text, kb = construct_message(
                magnet_list, desc_list, offset + items_per_page
            )
