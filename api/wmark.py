...
def wmark(update: "telegram.Update", context: "telegram.ext.CallbackContext") -> None:
    ...
    if not (
        message.reply_to_message and message.reply_to_message.photo and context.args
    ):
        message.reply_text(
        ...
        with BytesIO(
            pic.get_file().download_as_bytearray()
        ) as pic_file, BytesIO() as wimage:
            try:
                ...
                if w > h:
                    ...
                    txt_layer = txt_layer.rotate(
                        90, center=(w / 2, h / 2), translate=(w / 2, 0)
                    )
