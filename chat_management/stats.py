...
def print_stats(
    ...
    if (
        check_table_exists(table_name=str(chat_id))
...
def print_gstats(
    ...
    if (
        check_table_exists(table_name=str(chat_id) + "_total")
        and update.effective_chat.type != "private"
    ):
        cursor = conn.cursor()
        ...
        # Query to get top 10 users by message count
        formula: str = (
            f"SELECT * FROM `{chat_id}_total` ORDER BY message_count DESC LIMIT 10"
        )
        cursor.execute(formula)
        rows = cursor.fetchall()
...
def increment_total(chat_id: str) -> None:
    if check_table_exists(table_name=chat_id):
        ...
        if not check_table_exists(table_name=chat_id + "_total"):
            formula: str = (
                f"CREATE TABLE IF NOT EXISTS `{chat_id}_total` ( "
...
def clear(_context: "telegram.ext.CallbackContext") -> None:
    ...
    cursor.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE '%_total'"
    )
...
def increment(
    ...
    if not check_table_exists(table_name=str(chat_id)):
        formula: str = (
            f"CREATE TABLE IF NOT EXISTS `{chat_id}` ( "
            "`user_name` VARCHAR(255) NOT NULL UNIQUE, "
            "`message_count` INT unsigned NOT NULL DEFAULT '0', "
            "PRIMARY KEY (`user_name`))"
        )
